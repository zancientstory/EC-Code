...
void USART2_IRQHandler(void)
{
  ...
  uint8_t tmp_flag = __HAL_UART_GET_FLAG(&huart2, UART_FLAG_IDLE); // 获取IDLE状�??
  if ((tmp_flag != RESET))                                         // 判断接收是否结束
  {
    // recv_end_flag = 1; //接收结束
    __HAL_UART_CLEAR_IDLEFLAG(&huart2); // 清楚标志�?
    ...
    HAL_UART_Receive_DMA(&huart2, Usart2Type.RX_pData, RECEIVE_SIZE); // �?启DMA
  }
...
void USART3_IRQHandler(void)
{
  ...
  uint8_t tmp_flag = __HAL_UART_GET_FLAG(&huart3, UART_FLAG_IDLE); // 获取IDLE状�??
  if ((tmp_flag != RESET))                                         // 判断接收是否结束
  {
    // recv_end_flag = 1; //接收结束
    __HAL_UART_CLEAR_IDLEFLAG(&huart3); // 清楚标志�?
    ...
    HAL_UART_Receive_DMA(&huart3, Usart3Type.RX_pData, RECEIVE_SIZE); // �?启DMA
  }
